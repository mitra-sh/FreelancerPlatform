...
public class SearchController extends BaseController {
    ...
    public ResponseEntity searchTicketsBySkills(@RequestBody SearchTicketRequest request) {
        ...
        List<ServiceRequest> serviceRequests = baseService.findAllTickets();
    ...
    public ResponseEntity searchServiceProvidersBySkills(@RequestBody SearchTicketRequest request) {
        ...
        HashMap<String, ServiceProvider> finalServiceProviders = new HashMap<String, ServiceProvider>();
        for (String skillString : request.getSkills()) {
            for (ServiceProvider serviceProvider : serviceProviders) {
                for (Skill skill : serviceProvider.getSkills()) {
                    if (skill.getSkillTitle().equals(skillString)) {
                        finalServiceProviders.put(serviceProvider.getUsername(), serviceProvider);
                    }
                }
            }
        }
        ...
        return ResponseEntity.status(HttpStatus.ACCEPTED).
                body(finalServiceProviders.values());
